...
for t in range(int(t_max/dt) + 1):
    ...
    if t == 100:
        ...
        plt.subplot(2, 1, 1)
        plt.plot(edges[:-1] + np.diff(edges)/2, flux*2.71*Ts/c, label='t=0.1')
    elif t == 450:
        plt.subplot(2, 1, 2)
        plt.plot(edges[:-1] + np.diff(edges)/2, nb/n, label='t=0.45')
        plt.subplot(2, 1, 1)
        plt.plot(edges[:-1] + np.diff(edges)/2, flux*2.71*Ts/c, label='t=0.45')
    elif t == 750:
        plt.subplot(2, 1, 2)
        plt.plot(edges[:-1] + np.diff(edges)/2, nb/n, label='t=0.75')
        plt.subplot(2, 1, 1)
        plt.plot(edges[:-1] + np.diff(edges)/2, flux*2.71*Ts/c, label='t=0.75')
    elif t == 1000:
        plt.subplot(2, 1, 2)
        plt.plot(edges[:-1] + np.diff(edges)/2, nb/n, label='t=1')
        plt.subplot(2, 1, 1)
        plt.plot(edges[:-1] + np.diff(edges)/2, flux*2.71*Ts/c, label='t=1')
    elif t == 2000:
        plt.subplot(2, 1, 2)
        plt.plot(edges[:-1] + np.diff(edges)/2, nb/n, label='t=2')
        plt.subplot(2, 1, 1)
        plt.plot(edges[:-1] + np.diff(edges)/2, flux*2.71*Ts/c, label='t=2')
...
plt.subplot(2, 1, 1)
plt.plot(edges[:-1] + np.diff(edges)/2, flux*2.71*Ts/c, label='t='+str(round(t_max, 2)))
# plt.plot(edges[:-1] + np.diff(edges)/2, ana, label='Analytical Solution')
plt.ylabel('Energy Density [keV/$cm^3$]')
plt.title('Energy Density and Ion Fraction over Time [ns]')
plt.legend()
plt.show()