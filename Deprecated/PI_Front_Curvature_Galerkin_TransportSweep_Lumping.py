...
sigma_s = 5.1e-27           # Scattering cross section [cm^2]
...
# (R/2)*alpha(R/2, np.pi, 1)*alpha(R/2, np.pi, 2)*alpha(R/2, np.pi, 3)*R*2*np.pi
# (R/2)*alpha(R/2, np.pi, 2)**3*(R)*2*np.pi
# (R/2)*alpha(R/2, np.pi, 3)**3*R*2*np.pi
# (R/2)*alpha(R/2, np.pi, 3)*alpha(R/2, np.pi, 2)**2*R*2*np.pi
# (R/2)*alpha(R/2, np.pi, 2)*alpha(R/2, np.pi, 3)**2*R*2*np.pi
...
for i in range(P):
    ...
    f[i, :] = const*np.ones((N, ))*(a*c*Ts**3/(2.71*1.602e-25))
    for j in range(M2):
            ...
            for k in range(P):
                for l in range(P):
                    if k != l:
                        ...
                        Q[k, l] = 0 #-kappa*np.sum(epsilon[k, l, :]*nb_guess[0, :]) + A[k, l]*(1 - mu[j]**2)**0.5
                        Q[k, k] += -kappa*np.sum(epsilon[k, l, :]*nb_guess[0, :]) + A[k, l]*(1 - mu[j]**2)**0.5
                    else:
            ...
            for j in range(N):
                ...
                for k in range(P):
                    for l in range(P):
                        if k != l:
                            # Contributions from other nodes (off diagonal)
                            Q[k, l] = 0 #-kappa*np.sum(epsilon[k, l, :]*nb_guess[i, :]) + A[k, l]*(1 - mu[j]**2)**0.5
                            Q[k, k] += -kappa*np.sum(epsilon[k, l, :]*nb_guess[i, :]) + A[k, l]*(1 - mu[j]**2)**0.5
                        else:
                            # Node being solved for (on diagonal)
        ...
        for i in range(M):
            ...
            for k in range(P):
                for l in range(P):
                    if k != l:
                        # Contributions from other nodes (off diagonal)
                        Q[k, l] = 0 #kappa*(np.sum(epsilon[k, :, l]*Phi[i, :]))
                        Q[k, k] += kappa*(np.sum(epsilon[k, :, l]*Phi[i, :]))
                    else:
                        # Node being solved for (on diagonal)
